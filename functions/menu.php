...
register_nav_menus(
	array(
		...
		'footer-nav'	=> __( 'Footer Menu', 'jointswp' ),		// Secondary nav in footer
...
function joints_top_nav() {
	wp_nav_menu(array(
		...
		'items_wrap'		=> '<ul id="%1$s" class="%2$s" data-responsive-menu="accordion medium-dropdown" data-submenu-toggle="true" data-hover-delay="0" data-closing-time="0" data-alignment="left">%3$s</ul>',
	...
	if ( $args->theme_location == 'social-links' ) {
		...
		foreach( $items as &$item ) {
			...
			if( $icon ) {
				...
				$item->title = '<span class="title-icon-wrap"><span class="icon" aria-hidden="true"><img class="style-svg" src="' . esc_url($icon['url']) . '" alt="' .  esc_attr($icon['alt']) . '" /></span><span class="show-for-sr">' . $item->title . '</span>';
